package com.bilgeadam.recepergan.server;
...
public class ServerTsv {
		...
		try (ServerSocket serverSocket = new ServerSocket(5555)) {
			...
			while (true) {
				...
				if (array[1].equals("movie")) {
					...
					ArthistTsvDetail result = fileReaderNameCsvTryWithResources(array[0]);
		...
		try (BufferedReader bufferedReader = new BufferedReader(new FileReader(path))) {
			...
			while ((satir = bufferedReader.readLine()) != null) {
				...
				if (satir.toLowerCase().contains(searchKey.toLowerCase())) {
					...
					ArrayList<MoviesCsvDto> list = ControllerClientSearchDatabase.getInstance()
	...
	public ArthistTsvDetail fileReaderNameCsvTryWithResources(String searchKey) {
		
		String satir = "";
		String path = "C:\\Users\\rcper\\OneDrive\\Desktop\\Ara Tatili Ödevi\\names.tsv";
		ArthistTsvDetail temp = null;
		try (BufferedReader bufferedReader = new BufferedReader(new FileReader(path))) {
			while ((satir = bufferedReader.readLine()) != null) {
				
				if (satir.toLowerCase().contains(searchKey.toLowerCase())) {
					StringTokenizer stringTokenizer = new StringTokenizer(satir, "\t");
					temp = new ArthistTsvDetail();
